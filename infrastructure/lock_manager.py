...
class LockManager:
    ...
    def __init__(self, default_timeout: float = 30.0):
        ...
        self._active_locks_per_entity: DefaultDict[uuid.UUID, Set[LockInfo]] = defaultdict(set)
    ...
    def lock_entity(self, entity_id: uuid.UUID, lock_type: LockType = LockType.READ, 
                   timeout: Optional[float] = None) -> Generator[LockInfo, None, None]:
        """
        ...
        try:
            ...
            if not acquired:
                ...
                raise TimeoutError(
                    f"Could not acquire {lock_type.value} lock for entity {entity_id} "
                    f"within {timeout}s (elapsed: {elapsed:.2f}s)"
                )
    ...
    def force_release_all_locks(self, entity_id: Optional[uuid.UUID] = None) -> None:
        """
