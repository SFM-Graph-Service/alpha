    ...
    def get_performance_metrics(self) -> CachePerformanceMetrics:
        ...
        if self._operation_times:
            metrics.average_response_time = sum(
                self._operation_times) / len(self._operation_times)
...
def monitor_cache_performance(
        caches: Dict[str, Any]) -> Dict[str, CachePerformanceMetrics]:
    ...
    for cache_name, cache in caches.items():
        try:
            ...
            if hasattr(stats, 'memory_usage_bytes'):
                metrics.memory_usage_mb = stats.memory_usage_bytes / \
                    (1024 * 1024)
...
def generate_cache_report(caches: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report['performance_metrics'] = {
        name: metrics.to_dict() for name,
        metrics in performance_metrics.items()}
