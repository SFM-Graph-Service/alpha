...
class MemoryUsageStats:
    ...
    def capture_current(cls) -> 'MemoryUsageStats':
        ...
        if not PSUTIL_AVAILABLE:
            logger.warning(
                "psutil not available, returning empty memory stats")
            return cls()
...
class EvictionStrategyBase(ABC):
    ...
    @abstractmethod
    def select_nodes_for_eviction(self,
                                  graph: EvictableGraph,
                                  target_count: int) -> List[uuid.UUID]:
...
class LRUEvictionStrategy(EvictionStrategyBase):
    ...
    def select_nodes_for_eviction(self,
        ...
        candidates = self.access_tracker.get_lru_nodes(
            target_count * 2)  # Get more candidates
        available_nodes = [
            node_id for node_id in candidates if node_id in graph.get_all_node_ids()]
...
class OldestFirstEvictionStrategy(EvictionStrategyBase):
    ...
    def select_nodes_for_eviction(self,
        ...
        # Sort by UUID (which includes timestamp info in some cases) or access
        # time
        sorted_nodes = sorted(
            all_nodes,
            key=lambda node_id: self.access_tracker.get_access_time(node_id) or 0)
...
class MemoryMonitor:
    ...
    def should_evict_nodes(
            self,
            memory_stats: Optional[MemoryUsageStats] = None) -> bool:
        """Check if node eviction should be triggered."""
        if memory_stats is None:
            memory_stats = self.check_memory_usage()

        return memory_stats.is_over_limit(
            self.memory_limit_mb * self.warning_threshold)
    ...
    def evict_nodes(self, graph: EvictableGraph, force: bool = False) -> int:
        ...
        nodes_to_evict = strategy.select_nodes_for_eviction(
            graph, target_count)
        ...
        if evicted_count > 0:
            ...
            logger.info(
                f"Evicted {evicted_count} nodes using {
                    self._current_strategy.value} strategy")
