...
class GraphChangeObserver(ABC):
    ...
    @abstractmethod
    def on_node_updated(
            self, node: Node, previous_state: Dict[str, Any]) -> None:
    ...
    @abstractmethod
    def on_relationship_updated(self,
                                relationship: Relationship,
                                previous_state: Dict[str,
                                                     Any]) -> None:
...
class GraphObservable:
    ...
    def _notify_relationship_added(self, relationship: Relationship) -> None:
        ...
        self._record_change(
            "relationship_added",
            {
                ...
                "kind": relationship.kind.name if hasattr(
                    relationship.kind,
                    'name') else str(
                    relationship.kind)})
        for observer in self._observers:
    ...
    def _notify_relationship_updated(
            self, relationship: Relationship, previous_state: Dict[str, Any]) -> None:
    ...
    def _record_change(self, change_type: str,
