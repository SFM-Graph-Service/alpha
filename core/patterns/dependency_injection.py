...
class DIContainer:
    ...
    def _get_service(
            self,
            service_type: Type[T],
            scope: Optional[ServiceScope] = None) -> T:
        """Internal method to get a service instance."""
        if service_type not in self._services:
            raise InjectionError(
                f"Service {
                    service_type.__name__} is not registered")
        ...
        if service_type in self._creation_stack:
            ...
            dependency_chain = " -> ".join(
                cls.__name__ for cls in self._creation_stack)
        ...
        if descriptor.lifecycle == LifecycleType.SINGLETON:
            if service_type in self._singletons:
                ...
                return self._apply_interceptors(
                    service_type, self._singletons[service_type])
        ...
        if descriptor.lifecycle == LifecycleType.SCOPED:
            if scope and service_type in scope.services:
                descriptor.access_count += 1
                return self._apply_interceptors(
                    service_type, scope.services[service_type])
    ...
    def _create_instance(
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param.default == inspect.Parameter.empty:
                # Required parameter
                dependency: Any = self._get_service(param_type, scope)
    ...
    def get_service_info(
            self,
            service_type: Type) -> Optional[ServiceDescriptor]:
        ...
        for service_type, descriptor in self._services.items():
            for dependency in descriptor.dependencies:
                if dependency not in self._services:
                    issues.append(
                        f"Service {
                            service_type.__name__} depends on unregistered service {
                            dependency.__name__}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self._metrics, "registered_services": len(
                self._services), "singleton_instances": len(
                self._singletons), "interceptors": len(
                self._interceptors), "decorators": len(
                    self._decorators), "middleware": len(
                        self._middleware), "event_handlers": sum(
...
def configure_global_container(
        configurator: Callable[[DIContainer], None]) -> None:
