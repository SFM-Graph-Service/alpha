...
class AsyncEventHandler(EventHandler):
    ...
    def handle(self, event: Event) -> None:
        ...
        raise NotImplementedError(
            "Async handlers should implement handle_async")
...
class EventBus:
    ...
    def __init__(self, max_event_history: int = 1000):
        ...
        self._error_handlers: List[Callable[[
            Exception, Event, EventHandler], None]] = []
    ...
    def subscribe(
        ...
        if any(h.get_handler_id() ==
               handler_id for h in self._handlers[event_type]):
            logger.warning(
                f"Handler {handler_id} already registered for event type {event_type}")
            return handler_id
        ...
        logger.debug(
            f"Registered handler {handler_id} for event type {event_type}")
        return handler_id
        ...
        if event_type in self._handlers:
            for handler in self._handlers[event_type]:
                if handler.get_handler_id() == handler_id:
                    ...
                    self._metrics["handlers_removed"] += 1
                    logger.debug(
                        f"Removed handler {handler_id} from event type {event_type}")
                    return True
    ...
    async def _process_event_async(self, event: Event) -> None:
        ...
        for handler in handlers:
            if not isinstance(handler, AsyncEventHandler):
                sync_tasks.append(
                    self._handle_sync_event_in_async(
                        handler, event))
    ...
    async def _handle_async_event(
        ...
        try:
            ...
            logger.error(
                f"Error in async handler {
                    handler.get_handler_id()}: {e}")
            self._handle_error(e, event, handler)
            self._update_handler_metadata(handler, e, start_time)

    async def _handle_sync_event_in_async(
            self, handler: EventHandler, event: Event) -> None:
    ...
    def add_error_handler(self, error_handler: Callable[[
                          Exception, Event, EventHandler], None]) -> None:
    ...
    def get_handler_metadata(self,
                             handler_id: Optional[str] = None) -> Union[EventHandlerMetadata,
                                                                        List[EventHandlerMetadata]]:
...
class MetricsEventHandler(EventHandler):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_event_times": {
                k: v.isoformat() for k,
                v in self.last_event_time.items()}}
