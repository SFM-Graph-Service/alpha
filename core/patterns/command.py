...
class AddNodeCommand(Command):
    ...
    def undo(self) -> bool:
        ...
        collection_name = self.graph._node_registry.get_collection_name(
            self.node)  # type: ignore  # Protected access needed for command pattern
        ...
        if self.node.id in collection:
            ...
            # type: ignore  # Protected access needed for command pattern
            if hasattr(
                    self.graph,
                    '_node_index') and self.node.id in self.graph._node_index:
                # type: ignore  # Protected access needed for command pattern
...
class RemoveNodeCommand(Command):
    ...
    def execute(self) -> bool:
        ...
        self.removed_relationships = self.graph.get_node_relationships(
            self.node_id)
        ...
        if self.node_id in collection:
            ...
            if hasattr(
                    self.graph,
                    '_node_index') and self.node_id in self.graph._node_index:
                # type: ignore  # Protected access needed for command pattern
...
class AddRelationshipCommand(Command):
    ...
    def get_description(self) -> str:
        """Get a description of this command."""
        return f"Add relationship {
            self.relationship.source_id} -> {
            self.relationship.target_id} ({
...
class RemoveRelationshipCommand(Command):
    ...
    def get_description(self) -> str:
        ...
        if self.removed_relationship:
            return f"Remove relationship {
                self.removed_relationship.source_id} -> {
                self.removed_relationship.target_id}"
...
class CommandManager:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        successful_commands = sum(
            1 for cmd in self._history if cmd.executed and not cmd.undone)
        failed_commands = len(
            [meta for meta in self._command_metadata.values() if meta.error_message])
