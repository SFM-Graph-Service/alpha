...
class NodeSerializer:
    ...
    @staticmethod
    def _handle_institution(institution: Institution,
    ...
    @staticmethod
    def _add_actor_kwargs(data: Dict[str, Any],
                          node_kwargs: Dict[str, Any]) -> None:
    ...
    @staticmethod
    def _add_institution_kwargs(
            data: Dict[str, Any], node_kwargs: Dict[str, Any]) -> None:
    ...
    @staticmethod
    def _add_flow_kwargs(data: Dict[str, Any],
                         node_kwargs: Dict[str, Any]) -> None:
...
class SFMGraphSerializer:
    ...
    def _serialize_pickle(
            graph: SFMGraph,
            format_type: StorageFormat) -> bytes:
    ...
    def deserialize_graph(
        ...
        try:
            logger.debug(
                "Deserializing graph with format '%s'. Data size: %d bytes",
                format_type,
                len(data))
            ...
            if format_type in [
                ...
                missing_keys = [
                    key for key in required_keys if key not in dict_data]
                if missing_keys:
                    logger.error(
                        "Missing required keys in graph data: %s",
                        missing_keys)
            ...
            logger.error(
                "Failed to deserialize graph. Format: '%s', Error: %s",
                format_type,
                str(e))
            raise SFMSerializationError(
                f"Failed to deserialize graph: {
                    str(e)}") from e
    ...
    def _dict_to_graph(data: Dict[str, Any]) -> SFMGraph:
        ...
        try:
            ...
            for key in required_keys:
                if key not in data:
                    raise ValueError(
                        f"Missing required key '{key}' in graph data: {data}")
            ...
            logger.debug(
                "Node collections during deserialization: %s",
                node_type_mapping)
            ...
            for collection_name, node_class, target_collection in node_type_mapping:
                ...
                if not isinstance(collection_data, dict):
                    raise ValueError(
                        f"Invalid data for collection '{collection_name}': " f"{collection_data}")
                if not collection_data:
                    logger.warning(f"Collection '{collection_name}' is empty.")
                for node_data in collection_data.values():
                    if not isinstance(node_data, dict):
                        raise ValueError(
                            f"Invalid node data in collection '{collection_name}': " f"{node_data}")
                    node: Node = NodeSerializer.dict_to_node(
            ...
            if not isinstance(relationships_data, dict):
                raise ValueError(
                    f"Invalid data for relationships: {relationships_data}")

            logger.debug(
                "Relationships during deserialization: %s",
                relationships_data)

            for rel_data in relationships_data.values():
                relationship = SFMGraphSerializer._dict_to_relationship(
                    rel_data)
    ...
    def _dict_to_relationship(data: Dict[str, Any]) -> Relationship:
        ...
        try:
            ...
            return Relationship(
                ...
                # Handle missing modified_at
                modified_at=data.get('modified_at', None)
            )
        except Exception as e:
            logger.error("Failed to deserialize relationship: %s", str(e))
            raise SFMSerializationError(
                f"Failed to deserialize relationship: {
                    str(e)}") from e
...
class SFMPersistenceManager:
    ...
    def _create_metadata(self,
                         ...
                         checksum: str,
                         format_type: StorageFormat) -> GraphMetadata:
            ...
            try:
                ...
                if version is None or version == latest_metadata.version:
                    ...
                    if not metadata_to_load:
                        logger.warning(
                            "Graph '%s' version %s not found", graph_id, version)
                        return None
                ...
                self._verify_checksum(
                    serialized_data, metadata_to_load, graph_id)

                graph = SFMGraphSerializer.deserialize_graph(
                    serialized_data, metadata_to_load.format)
    ...
    def _verify_checksum(self, serialized_data: bytes, metadata: GraphMetadata,
        ...
        if metadata.checksum:
            ...
            if actual_checksum != metadata.checksum:
                logger.warning(
                    "Checksum mismatch for graph '%s'. Data may be corrupted.",
                    graph_id)
    ...
    def delete_graph(
        ...
        with self._thread_safe():
            try:
                ...
                # Delete metadata
                metadata_file = self.file_manager.get_metadata_file_path(
                    graph_id)
                ...
                logger.error(
                    "Failed to delete graph '%s': %s",
                    graph_id,
                    str(e))
                return False

    def list_graphs(
            self, include_metadata: bool = True) -> Sequence[Union[str, GraphMetadata]]:
        """
    ...
    def _count_backups(self, stats: Dict[str, Any]) -> None:
        ...
        if not self.file_manager.backups_path.exists():
            ...
            stats['backup_age_stats'] = {
                'oldest': None, 'newest': None, 'average_age_days': 0}
        ...
        for backup_file in backup_files:
            try:
                ...
                if file_size > 0:
                    try:
                        # Try to read first few bytes to validate file
                        # integrity
                        with backup_file.open('rb') as f:
                            # Read first 10 bytes to verify readability
                            f.read(10)
                        valid_count += 1
                    except (IOError, OSError):
                        logger.warning(
                            "Backup file %s appears corrupted", backup_file)
                        continue
                ...
                logger.warning(
                    "Failed to process backup file %s: %s", backup_file, e)
                continue
        ...
        try:
            ...
            if version is None and graph_id in self._metadata_cache:
                ...
                if self._validate_metadata_consistency(cached_metadata):
                    ...
                    logger.warning(
                        "Removed inconsistent cached metadata for '%s'", graph_id)

            metadata_data = self._load_metadata_data(graph_id, version)
            if not metadata_data:
                logger.debug(
                    "No metadata found for graph '%s', version %s",
                    graph_id,
                    version)
                return None
            ...
            if not self._validate_metadata_structure(metadata_data):
                logger.error(
                    "Invalid metadata structure for graph '%s'",
                    graph_id)
                return None
            ...
            if not self._validate_parsed_metadata(metadata, graph_id, version):
                logger.error(
                    "Parsed metadata validation failed for graph '%s'",
                    graph_id)
                return None
            ...
            logger.error(
                "JSON decode error loading metadata for '%s': %s",
                graph_id,
                str(e))
            return None
        except Exception as e:
    ...
    def _validate_metadata_consistency(self, metadata: GraphMetadata) -> bool:
        ...
        try:
            ...
            graph_file = self.file_manager.get_graph_file_path(
                metadata.graph_id, metadata.format)
            if not graph_file.exists():
                return False
            ...
            if abs(
                    actual_size -
                    metadata.size_bytes) > 100:  # Allow small variance for compression
                logger.warning(
                    "Size mismatch for graph '%s': expected %d, actual %d",
                    metadata.graph_id,
                    metadata.size_bytes,
                    actual_size)
                return False
            ...
            if metadata.modified_at and abs(
                    (file_mtime - metadata.modified_at).total_seconds()) > 60:
                logger.warning(
                    "Modification time mismatch for graph '%s'",
                    metadata.graph_id)
                return False
            ...
            logger.warning(
                "Error validating metadata consistency for '%s': %s",
                metadata.graph_id,
                e)
            return False

    def _validate_metadata_structure(
            self, metadata_data: Dict[str, Any]) -> bool:
        ...
        # Validate data types
        if not isinstance(
                metadata_data['graph_id'],
                str) or not metadata_data['graph_id']:
            logger.error("Invalid graph_id in metadata")
            return False

        if not isinstance(
                metadata_data['version'],
                int) or metadata_data['version'] < 1:
            logger.error(
                "Invalid version in metadata: %s",
                metadata_data['version'])
            return False

        if not isinstance(
                metadata_data['size_bytes'],
                int) or metadata_data['size_bytes'] < 0:
            logger.error(
                "Invalid size_bytes in metadata: %s",
                metadata_data['size_bytes'])
            return False
    ...
    def _validate_parsed_metadata(self, metadata: GraphMetadata, graph_id: str,
        ...
        if metadata.graph_id != graph_id:
            logger.error(
                "Graph ID mismatch: expected '%s', got '%s'",
                graph_id,
                metadata.graph_id)
            return False
        ...
        if metadata.checksum:
            try:
                ...
                if graph_file.exists():
                    ...
                    if actual_checksum != metadata.checksum:
                        logger.warning(
                            "Checksum mismatch for graph '%s': expected %s, got %s",
                            metadata.graph_id,
                            metadata.checksum,
                            actual_checksum)
                        # Don't fail validation for checksum mismatch, just
                        # warn
            except (OSError, IOError) as e:
                logger.warning(
                    "Could not validate checksum for '%s': %s",
                    metadata.graph_id,
                    e)
    ...
    def _archive_version(self, graph_id: str, metadata: GraphMetadata) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to archive version for '%s': %s",
                graph_id,
                str(e))
            raise
    ...
    def _validate_graph(self, graph: SFMGraph) -> None:
        ...
        if not (
            isinstance(
                graph.id, (str, uuid.UUID)) and str(
                graph.id).strip()):
            raise SFMSerializationError(
                "Graph ID must be a non-empty string or valid UUID")
        ...
        for collection_name, collection in node_collections:
            if not isinstance(collection, dict):
                raise SFMSerializationError(
                    f"Graph {collection_name} must be a dictionary")
            ...
            for node_id, node in collection.items():
                ...
                if not (isinstance(node_id, (str, uuid.UUID))
                    ...
                    raise SFMSerializationError(
                        f"Invalid node ID in {collection_name}: {node_id}")

                if not hasattr(node, 'id') or str(node.id) != str(node_id):
                    raise SFMSerializationError(
                        f"Node ID mismatch in {collection_name}: key '{node_id}' != node.id '{
                            getattr(
                                node, 'id', 'missing')}'")
        ...
        for rel_id, rel in graph.relationships.items():
            ...
            if not (isinstance(rel_id, (str, uuid.UUID))
                    and str(rel_id).strip()):
                raise SFMSerializationError(
                    f"Invalid relationship ID: {rel_id}")

            if not hasattr(rel, 'id') or str(rel.id) != str(rel_id):
                raise SFMSerializationError(
                    f"Relationship ID mismatch: key '{rel_id}' != rel.id '{
                        getattr(
                            rel, 'id', 'missing')}'")
            ...
            if not hasattr(
                    rel,
                    'source_id') or source_id_str not in all_node_ids:
                raise SFMSerializationError(
                    f"Relationship {rel_id} references non-existent source node: {
                        getattr(
                            rel, 'source_id', 'missing')}")
            if not hasattr(
                    rel,
                    'target_id') or target_id_str not in all_node_ids:
                raise SFMSerializationError(
                    f"Relationship {rel_id} references non-existent target node: {
                        getattr(
                            rel, 'target_id', 'missing')}")

            # Validate relationship has required attributes
            if not hasattr(rel, 'kind'):
                raise SFMSerializationError(
                    f"Relationship {rel_id} missing 'kind' attribute")

        # Additional integrity checks
        self._check_graph_structure_integrity(graph, all_node_ids)

    def _check_graph_structure_integrity(
            self, graph: SFMGraph, all_node_ids: set) -> None:
        """Perform additional graph structure integrity checks."""
        # Check for orphaned relationships (relationships with no corresponding
        # nodes)
        ...
        if total_nodes > 0 and total_relationships > total_nodes * 10:
            logger.warning(
                "Graph '%s' has unusually high relationship-to-node ratio: %d relationships for %d nodes",
                graph.id,
                total_relationships,
                total_nodes)
        ...
        if hasattr(
                graph,
                'description') and graph.description is not None and not isinstance(
                graph.description,
                str):
            raise SFMSerializationError(
                "Graph description must be a string or None")

    def _count_nodes(self, graph: SFMGraph) -> int:
        """Count total nodes in graph."""
        return (len(graph.actors) +
                len(graph.institutions) +
                len(graph.resources) +
                len(graph.processes) +
                len(graph.flows) +
                len(graph.policies) +
                len(graph.belief_systems) +
                len(graph.technology_systems) +
                len(graph.indicators) +
                len(graph.feedback_loops) +
                len(graph.system_properties) +
                len(graph.analytical_contexts))
    ...
    def get_version_history(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        # Iterate over version metadata files v{version}.json (but not
        # v{version}_data.json)
        for metadata_file in sorted(version_dir.glob("v[0-9]*.json")):
            ...
            try:
                ...
                logger.error(
                    "Failed to load version metadata from %s: %s",
                    metadata_file,
                    e)
        return history
    ...
    def create_backup(
        ...
        with self._thread_safe():
            try:
                ...
                if not metadata:
                    raise SFMPersistenceError(
                        f"Graph '{graph_id}' not found for backup.")

                graph_file = self.file_manager.get_graph_file_path(
                    graph_id, metadata.format)
                if not graph_file.exists():
                    raise SFMPersistenceError(
                        f"Graph file '{graph_file}' not found for backup.")

                backup_name = backup_name or f"{graph_id}_backup"
                backup_path = self.file_manager.backups_path / \
                ...
                serialized_data = graph_file.read_bytes()
                logger.debug("Serialized data in graph file: %s",
                             serialized_data[:100])

                shutil.copy2(graph_file, backup_path)
                logger.info(
                    "Backup created for graph '%s' at '%s'",
                    graph_id,
                    backup_path)
                ...
                logger.error(
                    "Failed to create backup for graph '%s': %s",
                    graph_id,
                    str(e))
                raise SFMPersistenceError(
        ...
        with self._thread_safe():
            try:
                ...
                if not backup_file.exists():
                    raise SFMPersistenceError(
                        f"Backup file '{backup_path}' not found.")
                ...
                # or reconstruct it. Here, we'll try to load the graph to
                ...
                logger.info(
                    "Graph '%s' restored successfully from backup '%s'",
                    new_graph_id,
                    backup_path)
                return new_graph_id
    ...
    def check_version_consistency(self, graph_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            current_graph_file = self.file_manager.get_graph_file_path(
                graph_id, current_metadata.format)
                ...
                if version_num:
                    ...
                    if not version_status['data_file_exists']:
                        ...
                        result['issues'].append(
                            f"Version {version_num} data file missing")
            ...
            if not result['is_consistent']:
                if "Current graph file missing" in result['issues']:
                    result['recommendations'].append(
                        "Restore current graph from latest version or backup")
                if any(
                        "data file missing" in issue for issue in result['issues']):
                    result['recommendations'].append(
                        "Consider cleaning up orphaned version metadata")

            return result

        except Exception as e:
            logger.error(
                "Failed to check version consistency for '%s': %s",
                graph_id,
                str(e))
            return {
                'graph_id': graph_id,
    ...
    def cleanup_old_versions(
            self, graph_id: str, keep_versions: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for version_info in versions_to_remove:
                ...
                try:
                    ...
                    version_dir = self.file_manager.versions_path / graph_id
                    version_metadata_file = version_dir / \
                    ...
                    logger.warning(
                        "Failed to remove version %d for graph '%s': %s",
                        version_num,
                        graph_id,
                        str(e))

            result['versions_after'] = result['versions_before'] - \
                len(result['cleaned_up'])

            logger.info("Cleaned up %d old versions for graph '%s', freed %d bytes", len(
                result['cleaned_up']), graph_id, result['space_freed_bytes'])

            return result

        except Exception as e:
            logger.error(
                "Failed to cleanup old versions for '%s': %s",
                graph_id,
                str(e))
            raise SFMPersistenceError(
                f"Failed to cleanup old versions: {
                    str(e)}") from e
    ...
    def cleanup_old_backups(self, max_age_days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            backup_files = list(
                self.file_manager.backups_path.glob("*.backup"))
            ...
            result['backups_after'] = result['backups_before'] - \
                len(result['cleaned_up'])
            ...
            raise SFMPersistenceError(
                f"Failed to cleanup old backups: {
                    str(e)}") from e
...
def load_sfm_graph(
        graph_id: str,
        storage_path: str = "./sfm_data") -> Optional[SFMGraph]:
