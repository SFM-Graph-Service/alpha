...
class PrometheusMetrics:
    ...
    def record_relationship_creation(self, relationship_type: str):
        ...
        self.relationships_created.labels(relationship_type=relationship_type).inc()
    ...
    def record_operation_duration(self, operation_type: str, operation_name: str, duration: float):
...
class MetricsCollector:
    ...
    def __init__(self, config: MetricConfig = None):
        ...
        self.prometheus_metrics = PrometheusMetrics() if self.config.prometheus_enabled else None
        self._lock = threading.Lock()
        self._custom_collectors: Dict[str, Callable] = {}
        
        if self.prometheus_metrics:
            self.logger.info("Metrics collector initialized with Prometheus support")
        else:
            self.logger.info("Metrics collector initialized without Prometheus support")
    ...
    def record_entity_operation(self, operation: str, entity_type: str, duration: float):
    ...
    def record_relationship_operation(self, operation: str, relationship_type: str, duration: float):
        ...
        if self.prometheus_metrics:
            if operation == "create":
                self.prometheus_metrics.record_relationship_creation(relationship_type)
            self.prometheus_metrics.record_operation_duration(
    ...
    def record_query(self, query_type: str, duration: float):
        ...
        # Record in Prometheus if available
        if self.prometheus_metrics:
            self.prometheus_metrics.record_query_execution(query_type, duration)
    
    def record_cache_operation(self, operation: str, cache_type: str):
        """Record cache operation metrics."""
        # Record in performance metrics
        self.performance_metrics.increment_counter(f"cache_{operation}_{cache_type}")
    ...
    def update_system_metrics(self, metrics: Dict[str, Any]):
        ...
        if self.prometheus_metrics:
            ...
            if 'active_connections' in metrics:
                self.prometheus_metrics.update_active_connections(metrics['active_connections'])
            
            # Update memory usage if provided
            if 'memory_usage' in metrics:
                for usage_type, bytes_used in metrics['memory_usage'].items():
                    self.prometheus_metrics.update_memory_usage(usage_type, bytes_used)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if operation_type == "entity" and entity_type:
                    metrics.record_entity_operation("execute", entity_type, duration)
                elif operation_type == "relationship" and entity_type:
                    metrics.record_relationship_operation("execute", entity_type, duration)
